...
class Shader:
    def __init__(self, tp, src):
        self.shader = glCreateShader(tp)
        glShaderSource(self.shader, src)
        glCompileShader(self.shader)
        if glGetShaderi(self.shader, GL_COMPILE_STATUS) == GL_FALSE:
...
class Program:
    def __init__(self, *shaders):
        ...
        for sh in shaders:
            glAttachShader(self.program, sh.shader)
        glLinkProgram(self.program)
        log = None
        if glGetProgrami(self.program, GL_LINK_STATUS) == GL_FALSE:
            log = glGetProgramInfoLog(self.program)
        for sh in shaders:
            glDetachShader(self.program, sh.shader)
        if log is not None:
    ...
    def attrib(self, name):
        return Attrib(self, glGetAttribLocation(self.program, name))

    def uniform(self, name):
        loc = GLuint(0)
        glGetUniformIndices(self.program, 1, [name], byref(loc))
        return Uniform(self, loc.value)

class Attrib:
    def __init__(self, program, location):
        self.program, self.location = program, location
...
class Buffer:
    ...
    def data(self, bits, usage=GL_DYNAMIC_DRAW, target=GL_ARRAY_BUFFER):
        self.bind(target)
        glBufferData(target, bits, usage)
